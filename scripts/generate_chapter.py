...
THREAD_LINES = [
    "真琴の頭から離れない未回収の問いは、{thread}という一点だった。今回の異変もその線上に置いてみると、偶然では説明しにくい位置でつながり始める。",
    "まだ片づいていない問題として残っているのは、{thread}という事実だ。目の前の異変だけを追うより、その糸がどこへ続いているかを見失わないことのほうが重要になりつつある。",
]
ANTAGONIST_LINES = [
